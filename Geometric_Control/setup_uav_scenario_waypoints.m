function Scenario = setup_uav_scenario_waypoints(pos_0, ori_0, ObstaclePositions, ObstacleHeights, ObstaclesWidths, Waypoints, Ts)
    ...
    n_Obstacles = size(ObstaclePositions, 1);
    n_Waypoints = size(Waypoints, 1);

    % Visualize the scenario with 3D rendering
    show3D(Scenario);
    hold on
    plot3([pos_0(1,2); Waypoints(:,2)],[pos_0(1,2); Waypoints(:,1)],[-pos_0(1,3); -Waypoints(:,3)],"--m")
    lgd = legend(["Start Position", ...
              "Obstacles", repmat("", 1, n_Obstacles - 1), ...
              "Waypoints", repmat("", 1, n_Waypoints - 1), ...
              "Direct Path"]);
    lgd.AutoUpdate = 'off';
